...
if FRAMEWORK_DIR:
    ...
    if not isfile(patchflag_path):
        ...
        if isfile(original_file):
            ...
            print(f"Warning: {original_file} not found. Skipping patch.")
else:
    print("Warning: framework-arduinoespressif32 not found. Skipping patch.")

FRAMEWORK_LIBS_DIR = env.PioPlatform().get_package_dir("framework-arduinoespressif32-libs") if "framework-arduinoespressif32-libs" in env.PioPlatform().packages else None
if FRAMEWORK_LIBS_DIR:
    # Add driver include paths for ESP-IDF 5.x drivers
    driver_subdirs = ["i2s", "dac", "rmt", "gpio"]
    for driver in driver_subdirs:
        driver_include_path = join(FRAMEWORK_LIBS_DIR, mcu, "include", "driver", driver, "include")
        if exists(driver_include_path):
            env.Append(CPPPATH=[driver_include_path])
            print(f"Added {driver.upper()} driver include path: {driver_include_path}")
        else:
            print(f"Warning: {driver.upper()} driver include path not found: {driver_include_path}")

    # Add HAL include path (for rmt_types.h, gpio_types.h, etc.)
    hal_include_path = join(FRAMEWORK_LIBS_DIR, mcu, "include", "hal", "include")
    if exists(hal_include_path):
        env.Append(CPPPATH=[hal_include_path])
        print(f"Added HAL include path: {hal_include_path}")
    else:
        print(f"Warning: HAL include path not found: {hal_include_path}")

    # Add SOC include path
    soc_include_path = join(FRAMEWORK_LIBS_DIR, mcu, "include", "soc", mcu, "include")
    if exists(soc_include_path):
        env.Append(CPPPATH=[soc_include_path])
        print(f"Added SOC include path: {soc_include_path}")
    else:
        print(f"Warning: SOC include path not found: {soc_include_path}")

    # Add ESP hardware support include path (for esp_etm.h)
    esp_hw_support_include_path = join(FRAMEWORK_LIBS_DIR, mcu, "include", "esp_hw_support", "include")
    if exists(esp_hw_support_include_path):
        env.Append(CPPPATH=[esp_hw_support_include_path])
        print(f"Added ESP HW Support include path: {esp_hw_support_include_path}")
    else:
        print(f"Warning: ESP HW Support include path not found: {esp_hw_support_include_path}")

    # Add Xtensa include path (for xt_utils.h)
    xtensa_include_path = join(FRAMEWORK_LIBS_DIR, mcu, "include", "xtensa", "include")
    if exists(xtensa_include_path):
        env.Append(CPPPATH=[xtensa_include_path])
        print(f"Added Xtensa include path: {xtensa_include_path}")
    else:
        print(f"Warning: Xtensa include path not found: {xtensa_include_path}")

    # Add ESP common include path (for esp_idf_version.h)
    esp_common_include_path = join(FRAMEWORK_LIBS_DIR, mcu, "include", "esp_common", "include")
    if exists(esp_common_include_path):
        env.Append(CPPPATH=[esp_common_include_path])
        print(f"Added ESP common include path: {esp_common_include_path}")
    else:
        print(f"Warning: ESP common include path not found: {esp_common_include_path}")
